...
function vitePluginManusDebugCollector(): Plugin {
  return {
    ...
    configureServer(server: ViteDevServer) {
      // API Endpoints in Dev Server
      server.middlewares.use("/api/leads", (req, res, next) => {
        const LEADS_FILE = path.join(PROJECT_ROOT, "server", "leads.json");
        ...
        if (req.method === "POST") {
          ...
          req.on("end", () => {
            try {
              const { nome, telefone, codigoImovel, tipoImovel, faixaInteresse } = JSON.parse(body);
              if (!nome || !telefone || !codigoImovel || !nome.trim() || !telefone.trim() || !codigoImovel.trim()) {
                res.writeHead(400, { "Content-Type": "application/json" });
                res.end(JSON.stringify({ error: "Por favor, preencha todos os campos obrigatórios." }));
                return;
              }
              ...
              if (!saved) {
                ...
                res.end(JSON.stringify({ error: "Erro ao salvar os dados no servidor." }));
                return;
              }
